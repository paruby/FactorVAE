...
class FactorVAE(object):
    ...
    def train(self):
        ...
        it=0
        while it < 300000:
            sess.run(self.ae_train_step, {self.input_ph: sample_minibatch()})
            sess.run(self.ae_train_step, {self.input_ph: sample_minibatch()})

            if it % 100 == 0:
                batch = sample_minibatch()
                ae_train_loss = sess.run(self.auto_encoder_loss, {self.input_ph: batch})
                recon_train_loss = sess.run(self.recon_loss, {self.input_ph: batch})
                disc_train_loss = sess.run(self.disc_loss, {self.input_ph: batch})
                print("Iteration %i: \n    Autoencoder loss (train) %d\n    Reconstruction loss (train) %s\n    Discriminator loss (train) %s" % (it, ae_train_loss, recon_train_loss, disc_train_loss), flush=True)

                ae_test_loss = sess.run(self.auto_encoder_loss, {self.input_ph: self.data_test[0:500]})
                recon_test_loss = sess.run(self.recon_loss, {self.input_ph: self.data_test[0:500]})
                disc_test_loss = sess.run(self.disc_loss, {self.input_ph: self.data_test[0:500]})
                print("    Autoencoder loss (test) %d\n    Reconstruction loss (test) %s\n    Discriminator loss (test) %s" % (it, ae_test_loss, recon_test_loss, disc_test_loss), flush=True)


    def sample_minibatch(self, batch_size=64):
    ...
    def _data_init(self):
        ...
        data_train = data[0 : (9*n_data)//10]
        data_test = data[(9*n_data)//10 : ]
    ...
    def _encoder_init(self, inputs):
        with tf.variable_scope("encoder"):
            e_1 = tf.layers.conv2d(inputs=inputs, filters=32, kernel_size=4, stride=2, activation=tf.nn.relu, name="e_1")
            e_2 = tf.layers.conv2d(inputs=e_1, filters=32, kernel_size=4, stride=2, activation=tf.nn.relu, name="e_2")
            e_3 = tf.layers.conv2d(inputs=e_2, filters=64, kernel_size=4, stride=2, activation=tf.nn.relu, name="e_3")
            e_4 = tf.layers.conv2d(inputs=e_3, filters=64, kernel_size=4, stride=2, activation=tf.nn.relu, name="e_4")
            e_5 = tf.layers.dense(inputs=e_4, units=128, name="e_5")
        ...
        return input_ph, e_mean, e_logvar
    ...
    def _decoder_init(self, inputs, reuse=False):
        with tf.variable_scope("decoder"):
            ...
            d_2 = tf.layers.dense(inputs=d_2, units=1024, activation=tf.nn.relu, name="d_2", reuse=reuse)
            d_2_reshape = tf.reshape(d_2, shape=[-1, 4, 4, 64])
            d_3 = tf.layers.conv2d_transpose(inputs=d_2_reshape, filters=64, kernel_size=4, stride=2, activation=tf.nn.relu, name="d_3", reuse=reuse)
            d_4 = tf.layers.conv2d_transpose(inputs=d_3, filters=32, kernel_size=4, stride=2, activation=tf.nn.relu, name="d_4", reuse=reuse)
            d_out = tf.layers.conv2d_transpose(inputs=d_4, filters=1, kernel_size=4, stride=2, name="d_out", reuse=reuse)
    ...
    def _loss_init(self):
        ...
        with tf.variable_scope("discriminator"):
            logits_real, probs_real = _discriminator_init(real_samples)
            logits_permuted, probs_permuted = _discriminator_init(permuted_samples, reuse=True)
